...
public class DnaExtractorBlock extends AbstractGeneticsBlock {
    ...
    public DnaExtractorBlock() {
        super(DnaExtractorBlockEntity::new);
    }

    @Override
    public void appendTooltip(ItemStack stack, @Nullable BlockView world, List<Text> tooltip, TooltipContext options) {
        tooltip.add(Text.translatable("tooltip.advancedgenetics.energy_requirement",  Config.Common.dnaExtractorEnergyPerTick.get()).formatted(Formatting.GRAY));
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (!world.isClient()) {
            return (level, pos, blockState, blockEntity) -> {
                if (blockEntity instanceof DnaExtractorBlockEntity extractorBlockEntity) {
