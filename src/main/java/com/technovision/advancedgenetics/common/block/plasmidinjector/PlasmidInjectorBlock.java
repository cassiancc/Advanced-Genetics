...
public class PlasmidInjectorBlock extends AbstractGeneticsBlock {
    ...
    public PlasmidInjectorBlock() {
        super(PlasmidInjectorBlockEntity::new);
    }

    @Override
    public void appendTooltip(ItemStack stack, @Nullable BlockView world, List<Text> tooltip, TooltipContext options) {
        tooltip.add(Text.translatable("tooltip.advancedgenetics.energy_requirement",  Config.Common.plasmidInjectorEnergyPerTick.get()).formatted(Formatting.GRAY));
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (!world.isClient()) {
            return (level, pos, blockState, blockEntity) -> {
                if (blockEntity instanceof PlasmidInjectorBlockEntity injectorBlockEntity) {
                    injectorBlockEntity.tick();
                }
            };
        }
        return null;
